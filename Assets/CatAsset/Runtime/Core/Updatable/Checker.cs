...
namespace CatAsset
{
    ...
    public class Checker
    {
        ...
        private void CheckRemoteManifest(bool success, string error, UnityWebRequest uwr)
        {
            ...
            string readWritePath = Util.GetReadWritePath(Util.ManifestFileName);
            Debug.Log("可读可写路径Manifest Path =" + readWritePath);
            CatAssetManifest manifest = JsonParser.ParseJson<CatAssetManifest>(uwr.downloadHandler.text);
            Dictionary<string, Hash128> localBundle = new Dictionary<string, Hash128>();
            /// 本地差异对比
            if (System.IO.File.Exists((Util.GetReadWritePath(Util.ManifestFileName))))
            {
                ...
                {
                    if (!localBundle.ContainsKey(item.BundleName))
                    {
                        localBundle.Add(item.BundleName, item.Hash);
                    }
                }
            }
            ...
            foreach (BundleManifestInfo item in manifest.Bundles)
            {
                if (localBundle.ContainsKey(item.BundleName) && item.Hash == localBundle[item.BundleName])
                {
                    continue;
                }
